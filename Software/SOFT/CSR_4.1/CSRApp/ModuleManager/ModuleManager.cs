            ...
            foreach (KeyValuePair<string, config.ModuleInfo> kvp in appConfig.Modules)
            {
                ...
                if (!available_module_names.ContainsKey(kvp.Key))
                {
                    ...
                    if (kvp.Value is config.ModuleInfoLocal)
                    {
                        ...
                        if (!assembly.Load())
                        {
                            Error("No se ha podido cargar el modulo \"" + kvp.Key + "\"!\nCompruebe que la ruta a la librería es correcta y que contiene algún tipo de módulo CSRModule o derivado.", true);
                            errors_found = true;
                        }
                        else
                        {
                    ...
                    CSRCarga.Progress = Convert.ToInt32((((float)ind) / ((float)appConfig.Modules.Count)) * 50);
                }
                else
        ...
        public bool InitModules(System.Drawing.Size size)
        {
            ...
            foreach (csr.modules.CSRModule module in available_local_modules.Values)
            {
                try
                {
                    ...
                    CSRStatusFunctions.Instance.SetModuleConnection(module.Id, true);

                    ConsoleWrite("Módulo " + module.Name + " cargado correctamente.", true);

                    CSRCarga.Progress = 50 + Convert.ToInt32((((float)ind) / ((float)appConfig.Modules.Count)) * 50);
                } 
                catch(Exception e)
                {
